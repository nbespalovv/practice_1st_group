...
def get_actor_by_name_and_coworkers(name: str):
    ...
    actors.append(actor)
    films = DB.get_films_by_actor(actor.id_actor)
    for film in films:
        related_actors = DB.get_actors_by_films(film.id_film)
        for related_actor in related_actors:
            if related_actor not in actors:
                actors.append(related_actor)

    return actors


def parse_actors(actors_db) -> list[ActorDTO]:
    actors = [ActorDTO(x.id_actor, x.name, [], []) for x in actors_db]
    for actor in actors:
        films = DB.get_films_by_actor(actor.actor_id)
        actor.films = [x.id_film for x in films]
        for film in actor.films:
            related_actors = DB.get_actors_by_films(film)
            for related_actor in related_actors:
                if related_actor.id_actor != actor.actor_id:
                    coworker = next(
                        (x for x in actors if x.actor_id == related_actor.id_actor), None)
                    if coworker is not None:
                        actor.coworkers.append(coworker)
    return actors

def draw_social_graph(actor_name: str):
    actors_from_db = get_actor_by_name_and_coworkers(actor_name)  # получаем объекты актеров из бд
    ...
    nx.draw(G, pos=position, node_color='yellow', edge_color='black', font_size=30, font_color='black',
            with_labels=True)  # отрисовка картиночки
    path=f'../all_graphs/graph-{datetime.datetime.now().strftime("%d-%m-%Y_%H-%M")}.png'
    plt.savefig(path)  # картиночка
    return path